...
define([
  ...
  return {
    ...
    initActions: function () {
      ...
      self.view.tbxAccountNumber.onEndEditing = function () {
        let accountNumber = self.view.tbxAccountNumber.text.trim();
        var specialPackages = [
          "Ethiopian Airlines",
          "Guzgo Airlines",
        ...
        if (specialPackages.includes(self.selectedPackage)) {
          if (self.selectedPackage === "US Visa") {
            if (!/^[a-zA-Z\s]+$/.test(accountNumber)) {
              self.showFieldError(
                "Passenger Name must contain only alphabets."
              );
              self.view.txtAmount.setEnabled(false);
              self.view.tbxAccountNumber.setFocus(true);
              return;
            ...
            if (!/^[0-9]+$/.test(accountNumber) || accountNumber.length !== 9) {
              self.showFieldError("Account number must be exactly 9 digits.");
              self.view.txtAmount.setEnabled(false);
              self.view.tbxAccountNumber.setFocus(true);
              return;
            }
          ...
          self.onlyNumbersAllowed();
          if (accountNumber.startsWith("+251")) {
            let numberWithoutCode = accountNumber.substring(4);
            if (numberWithoutCode.length !== 9) {
              self.showFieldError(
                "Phone number must be exactly 9 digits after the country code +251."
              );
              self.view.txtAmount.setEnabled(false);
              self.view.tbxAccountNumber.setFocus(true);
              return;
        ...
        // Proceed with specific package API calls
        if (self.selectedPackage === "Ethio Telecom Postpaid") {
          if (accountNumber.startsWith("+251")) {
          ...
          var payload = {
            RequestID: "888100",
            BillerID: "1112",
            BillRefNumber: self.selectedPackageId + "|" + accountNumber,
          };
      ...
      //             self.showFieldError("Your account number must be between 9 and 10 digits after the country code +251.");
      //             self.view.txtAmount.setEnabled(false);
      //             self.view.tbxAccountNumber.setFocus(true);
      //             return;
      //         }
      //     } else {
      //         self.showFieldError("Account number must start with country code +251.");
      //         self.view.txtAmount.setEnabled(false);
      //         self.view.tbxAccountNumber.setFocus(true);
      //         return;
      //     }
      ...
      //         }
      //         self.presenter.getDSTVUserDetails(payload, self.dstvDetailsSuccess.bind(self), self.accountDetailsFailure.bind(self));
      //     } else if(self.selectedPackage === "Water Bill"){
      //         var payload = {
      //             "biller_id": self.selectedCityID,
      //             "bill_id": self.view.tbxAccountNumber.text.trim()
      //         }
      //         self.presenter.getWaterBillDetails(payload, self.waterBillDetails.bind(self), self.accountDetailsFailure.bind(self));
      //     } else if(self.selectedPackage === "School Fees"){
      ...
      //             "Type": 0
      //         }
      //         self.presenter.getStudentDetailName(payload, self.getStudentDetailNameSuccess.bind(self), self.accountDetailsFailure.bind(self));
      //     }
      // };
    ...
    //         if (numberWithoutCode.length < 9 || numberWithoutCode.length > 10) {
    //             self.showFieldError("Phone number must be between 9 and 10 digits after country code +251.");
    //             self.view.txtAmount.setEnabled(false);
    //             self.view.tbxAccountNumber.setFocus(true);
    //             self.view.lblWarning.setVisibility(true);
    //             return;
    ...
    //             self.showFieldError("Account number must be between 9 and 11 digits.");
    //             self.view.txtAmount.setEnabled(false);
    //             self.view.tbxAccountNumber.setFocus(true);
    //             self.view.lblWarning.setVisibility(true);
    //             return;
    ...
    onlyNumbersAllowed: function () {
      var self = this;
      var input = self.view.tbxAccountNumber.text.trim();
      var specialPackages = [
        "Ethiopian Airlines",
        "Guzgo Airlines",
        "US Visa",
      ...
      var isValidNumberFormat = /^[0-9]+$/.test(input);

      if (specialPackages.includes(self.selectedPackage)) {
        // For Ethiopian Airlines, Guzgo Airlines, US Visa, and DSTV
        if (self.selectedPackage === "US Visa") {
          // US Visa expects a passenger name, not a number
          ...
          // For Ethiopian Airlines, Guzgo Airlines, and DSTV, expect exactly 9 digits
          if (!isValidNumberFormat || input.length !== 9) {
            self.showFieldError("Account number must be exactly 9 digits.");
            self.view.txtAmount.setEnabled(false);
            self.view.tbxAccountNumber.setFocus(true);
            self.view.lblWarning.setVisibility(true);
        ...
        // For other packages (e.g., Ethio Telecom, Water Bill, School Fees)
        var isValidFormat = /^[0-9+]+$/.test(input);
        if (!isValidFormat) {
          self.showFieldError("Only numbers and '+' are allowed.");
          self.view.txtAmount.setEnabled(false);
        ...
        if (input.startsWith("+251")) {
          ...
          if (numberWithoutCode.length !== 9) {
            self.showFieldError(
              "Phone number must be exactly 9 digits after country code +251."
            );
            self.view.txtAmount.setEnabled(false);
            self.view.tbxAccountNumber.setFocus(true);
            self.view.lblWarning.setVisibility(true);

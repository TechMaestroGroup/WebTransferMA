define([
      ...
      for (var key in keyValues) {
        if (widget === key) {
          ...
          if (typeof fieldValue === "string") {
            ...
            return kony.i18n.getLocalizedString(
              data.substring(data.indexOf("${i18n{") + 7, data.length - 2)
            );
          }
        }
      }
   ...
   * @return : NA
   */
  BusinessController.prototype.invokeCustomVerbforCreateTransaction =
    function () {
      var scope = this;
      var collectionObj = this.store.getState();
      var serviceName = "";
      var transferType =
        collectionObj.Collection["Transaction"]["transferType"];
      ...
      if (criteria["transactionAmount"])
        criteria["transactionAmount"] = this.getDeformattedAmount(
      ...
      scope.invokeServiceUtils
        ...
        .then(
          this.setAcknowledgementInCollection.bind(
            this,
            this.serviceParameters[serviceName].Object
          )
        )
        .catch(
          scope.setError.bind(this, "invokeCustomVerbforCreateTransaction")
        );
    };
   ...
   * @return : NA
   */
  BusinessController.prototype.invokeCustomVerbforEditTransaction =
    function () {
      var scope = this;
      var collectionObj = this.store.getState();
      var serviceName = "";
      ...
      if (
        ...
        if (transferType === "Same Bank") {
          if (accountType === "External")
            serviceName = "IntraBankAccFundTransferEdit";
          else serviceName = "TransferToOwnAccountsEdit";
        } else if (transferType === "Domestic Transfer")
          serviceName = "InterBankFundTransferEdit";
        else if (transferType === "International Transfer")
          serviceName = "InternationalFundTransferEdit";
      } else {
        serviceName = "CreateOneTimeTransfer";
      }
      kony.application.showLoadingScreen(
      ...
      if (!criteria["payeeCurrency"])
        criteria["payeeCurrency"] = criteria["transactionCurrency"];
      if (criteria["amount"])
        criteria["amount"] = this.getDeformattedAmount(criteria["amount"]);
      if (criteria["transactionAmount"])
        criteria["transactionAmount"] = this.getDeformattedAmount(
   ...
   * @return : NA
   */
  BusinessController.prototype.setAcknowledgementInCollection = function (
    object,
    data
  ) {
    var scope = this;
    ...
    if (
      ...
      scope.dataJSON["serviceName"] =
        collectionObj.Collection[objectName].serviceName;
  ...
  BusinessController.prototype.mpesaPayment = function (
    data,
    onSuccess,
    onError
  ) {
  ...
  BusinessController.prototype.mpesaTrustPayment = function (
    data,
    onSuccess,
    onError
  ) {
  ...
  BusinessController.prototype.telebirrPayment = function (
    data,
    onSuccess,
    onError
  ) {
  ...
  BusinessController.prototype.aTMPayment = function (
    data,
    onSuccess,
    onError
  ) {

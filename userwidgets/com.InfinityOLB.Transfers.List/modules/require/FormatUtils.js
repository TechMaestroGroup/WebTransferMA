define(function () {
  ...
  /**
    ...
    */
  FormatUtils.prototype.formatAmountandAppendCurrencySymbol = function (amount,currencySymbolCode,FormatData) {
    ...
    var currencySymbol = "";
    if((Number(amount))>=0){
       formattedAmount=FormatData.positiveFormat;
      if(FormatData && formattedAmount.indexOf("{CS}")!==-1){
        currencySymbol = this.getCurrencySymbol(currencySymbolCode);
        formattedAmount= formattedAmount.replace("{CS}",currencySymbol);
      }
      else{
        currencySymbol = currencySymbolCode;
        formattedAmount= formattedAmount.replace("{CT}",currencySymbol);
      }
      formattedAmount= formattedAmount.replace("{D}",amountWithSeperator);
      return formattedAmount;
    }
    else{
       formattedAmount=FormatData.negativeFormat;
      if(FormatData && formattedAmount.indexOf("{CS}")!==-1){
        currencySymbol = this.getCurrencySymbol(currencySymbolCode);
        formattedAmount= formattedAmount.replace("{CS}",currencySymbol);
      }
      ...
      formattedAmount= formattedAmount.replace("{D}",amountWithSeperator.split('-')[1]);
      return formattedAmount;
  ...
  FormatUtils.prototype.formatText = function (value, type, FormatvalueJSON) {
    var scopeObj = this;
    var fieldType = type;
    ...
    if (fieldValue) {
      if (fieldType == "amount") {
        var formattedAmount = "";
        formattedAmount = this.formatAmountandAppendCurrencySymbol(fieldValue, FormatJSON.currencyCode, FormatJSON.amountFormat);
        return formattedAmount;
      } else if (fieldType == "date") {
        if (FormatJSON.dateFormat && FormatJSON.dateFormat.format) {
          var dateObjFromString = this.getDateObjectfromString(value)
          return this.getFormatedDateString(dateObjFromString, FormatJSON.dateFormat.format);
        }
      } else if (fieldType == "Account Number") {
        if (FormatJSON.masking) {
          if (FormatJSON.accountNumberformat) {
            FormatJSON.accountNumberformat.format = (!FormatJSON.accountNumberformat.format) ? "\\d(?=\\d{4})" : FormatJSON.accountNumberformat.format;
            FormatJSON.accountNumberformat.replaceCharacter = (!FormatJSON.accountNumberformat.replaceCharacter) ? "X" : FormatJSON.accountNumberformat.replaceCharacter;
            FormatJSON.accountNumberformat.modifiers = (!FormatJSON.accountNumberformat.modifiers) ? "g" : FormatJSON.accountNumberformat.modifiers;
          }
          return this.formatAccoutNumber(FormatJSON.accountNumberformat, fieldValue);
        }
      } else if (fieldType == "AccountNumberWithName") {
        var accountNumber, accountName, digitsLength = JSON.parse(FormatJSON.accountNumberLength);
        fieldValue.forEach(function (item) {
          accountNumber = accountNumber || item["accountNumber"];
          accountName = accountName || item["accountName"];
        });
        if (!accountName) return accountNumber.slice(-digitsLength);
        return accountName + "..." + accountNumber.slice(-digitsLength);
      } else if (fieldType == "Percentage") {
        return this.formatValueAndAppendPercentageSymbol(fieldValue);
      } else if (fieldType == "AmountWithTransactionCurrency") {
        var formattedAmount = "";
        formattedAmount = this.formatAmountandAppendCurrencySymbol(fieldValue["amount"], fieldValue["currencyCode"], FormatJSON.amountFormat);
        return formattedAmount;
      } else {
        return fieldValue;
      }
